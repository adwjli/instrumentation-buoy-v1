...
void ADXL345::powerOn() {
	...
	writeTo(ADXL345_POWER_CTL, 16); // Enable Auto-save
	writeTo(ADXL345_POWER_CTL, 8); // Enable measurement mode
}
...
int ADXL345::readAccelX() {
	readFrom(ADXL345_DATAX0, ADXL345_TO_READ, _buff);
	return (((int)_buff[1]) << 8) | _buff[0];
}

int ADXL345::readAccelY() {
	readFrom(ADXL345_DATAX0, ADXL345_TO_READ, _buff);
	return (((int)_buff[3]) << 8) | _buff[2];
}

int ADXL345::readAccelZ() {
	readFrom(ADXL345_DATAX0, ADXL345_TO_READ, _buff);
	return (((int)_buff[5]) << 8) | _buff[4];
}
